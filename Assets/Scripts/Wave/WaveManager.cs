...
public class SpawnOption
{
    ...
    public int weight = 1;    // 가중치(양수). 클수록 잘 뽑힘.
}
/// <summary>
...
public class WaveManager : MonoBehaviour
{
    ...
    GameObject PickRandomPrefab()
    {
        ...
        while (i < options.Length)
        {
            if (options[i] != null)
            {
                total = total + Mathf.Max(0, options[i].weight);
            }
            i = i + 1;
        }
        if (total <= 0)
        ...
        return options[options.Length - 1].prefab;
    }

    void SpawnOne()
    {
        GameObject pf = PickRandomPrefab();
        if (pf == null)
        {
            return;
        }
        ...
        if (tracker == null)
        {
            tracker = go.AddComponent<EnemyTracker>();
        }
